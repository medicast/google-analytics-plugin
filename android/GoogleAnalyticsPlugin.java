...
public class GoogleAnalyticsPlugin extends CordovaPlugin {
  ...
  public boolean execute(String action, String rawArgs, CallbackContext callback)
    ...
    } else if ("getLimitAdTracking".equals(action)) {
      getLimitAdTracking(callback);
      return true;
    ...
    } else if ("setScreen".equals(action)) {
      setScreen(rawArgs, callback);
      return true;
  ...
  // Methods that correspond to plugin JS function calls
  // Also see execute() method above
  ...
  private void dispatch(final CallbackContext callbackContext) {
    cordova.getThreadPool().execute(new Runnable() {
      public void run() {
        GoogleAnalyticsPlugin plugin = GoogleAnalyticsPlugin.this;
        if (plugin.hasTracker(callbackContext)) {
  ...
  private void getAdvertisingId(CallbackContext callbackContext) {
    if (hasTracker(callbackContext)) {
      try {
        String value = AdvertisingIdClient.getAdvertisingIdInfo(cordova.getActivity()).getId();
        callbackContext.success(value);
      } catch (Exception e) {
        callbackContext.error(e.toString());
      }
  ...
  private void getLimitAdTracking(CallbackContext callbackContext) {
    if (hasTracker(callbackContext)) {
      try {
        boolean value = AdvertisingIdClient.getAdvertisingIdInfo(cordova.getActivity()).isLimitAdTrackingEnabled();
  ...
  private void setDispatchInterval(String rawArgs, CallbackContext callbackContext) {
    try {
  ...
  private void setIDFAEnabled(String rawArgs, CallbackContext callbackContext) {
    if (hasTracker(callbackContext)) {
      try {
        tracker.enableAdvertisingIdCollection(new JSONArray(rawArgs).getBoolean(0));
        callbackContext.success();
      } catch (JSONException e) {
        callbackContext.error(e.toString());
      }
  ...
  private void setScreen(String rawArgs, CallbackContext callbackContext) {
    if (hasTracker(callbackContext)) {
      try {
        String screenName = new JSONArray(rawArgs).getString(0);
